# Record of AI Prompts Used in Code Development

**Manuscript:** Context-Aware Dynamic Tool Resolution for Deadline-Constrained Autonomous Agents
**Authors:** Muhammet Ali Öztürk, Harun Artuner (Hacettepe University, Ankara, Türkiye)

This document records the prompts used with AI coding assistants during the
development of the simulation code, as declared in the manuscript
(Section "Use of AI-assisted tools"). The core algorithms, mathematical
formulation, and experimental design were specified by the authors before
implementation; AI tools were used under the authors' direction to translate
these specifications into code and to restructure the repository. The authors
reviewed, verified, and take full responsibility for all AI-assisted output.

The complete development history is publicly available at
<https://github.com/maliozturk/Deadline_Aware_Tool_Permissioning>, so every
before/after state referenced below is independently verifiable by commit hash.

## Timeline of AI-assisted code work

| Phase | Date | Tool | Code before | Code after | Prompt record |
|---|---|---|---|---|---|
| 1. Initial implementation: translation of author-specified algorithms into the object-oriented simulation framework | Jan 2026 | OpenAI Codex 5.3 (via Google Antigravity) | Hand-written prototype scripts (predate version control; not retained) | Commit `9183743` (2026-01-18, earliest retained code) | Not retained; representative prompt reconstructed in Section 1 |
| 2. Journal-revision coder-agent sequence (J-mode generalization, trace collection, calibration, baselines, experiments) | May 2026 | OpenAI Codex 5.4 (via Google Antigravity) | Commit `3aad67e` (2026-03-13) | Commit `d816dec` (2026-05-31) | **Verbatim**, Section 2 — this prompt file was written by the authors and committed to the repository at the time (`revision/02_coder_agent_prompts.md`, added in commit `024aa18`) |
| 3. Final repository restructuring for release | 2026-06-27 | Anthropic Claude Opus 4.8 (via Claude Code) | Commit `d816dec` | Commit `9b630fb` (commit carries the `Co-Authored-By: Claude Opus 4.8` trailer) | Session logs auto-purged by the tool's retention policy; reconstructed in Section 3; the executed scope is documented verbatim in the commit message of `9b630fb` |

---

## Section 1 — Initial implementation (January 2026)

> **Status: reconstructed from memory.** The original session logs were not
> retained. The prompt below reflects, to the best of the authors' recollection,
> the instruction given to the coding assistant, which was accompanied by the
> authors' hand-written prototype scripts and the mathematical specification of
> the FTC/CADTR policies.

> *"Take my simulation code and make it systematic and reusable: convert it to
> an object-oriented architecture with a clear separation between the simulator
> core (event loop, queue, tasks), the decision policies, the service-time and
> arrival distributions, and the metrics collection. Make every experiment
> configurable from a single configuration module so that reviewers can easily
> run it. Implement the policies exactly as specified in my notes; do not change
> the mathematics."*

The result of this phase is the earliest retained version of the codebase
(commit `9183743`, 2026-01-18), which already reflects this structure
(`Core/`, `Models/`, `Metrics/`, `Configurations.py`).

---

## Section 2 — Coder-agent prompt sequence (May 2026) — VERBATIM

> **Status: verbatim.** The following prompt file was authored by us, issued to
> the coder agent one prompt at a time, and committed to the repository while
> the work was in progress (`revision/02_coder_agent_prompts.md`, commit
> `024aa18`; the commits `p1 done`, `p2 done`, `p3 done` track its execution).
> It is reproduced here unchanged.
...
## Section 3 — Final repository restructuring (27 June 2026)

> **Status: reconstructed.** This session was run in Anthropic Claude Code,
> whose local transcripts are automatically deleted after 30 days; the exact
> prompt text is therefore no longer available. The reconstruction below is
> corroborated by the commit message of `9b630fb`, which documents the executed
> scope in detail and carries the `Co-Authored-By: Claude Opus 4.8` trailer.

> *"Clean the repository for the PeerJ release. Scope it to the published
> two-mode (J=2) FTC/CADTR study; normalize the naming from DATP/TFG to
> FTC/CADTR without changing behavior (verify results are byte-identical);
> archive the journal-era scripts and the J=3 apparatus; add a `reproduce/`
> pipeline (run_sweep, make_figures, make_multimodel_ecdf) that reproduces
> every paper figure and table exactly; add `requirements.txt`, a regression
> test, and a README mapping each figure and table to a command."*

---

## Note on scope

AI writing assistance for the manuscript text (grammar and phrasing, and
porting to the journal LaTeX template) is declared in the manuscript's
Acknowledgements and Section "Use of AI-assisted tools"; it did not alter the
scientific content. This document covers the computer code, as requested by
the journal.
